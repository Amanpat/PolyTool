...
def _marker_metadata(arxiv_id="2604.24366", body_source="marker") -> str:
...
class TestQueryAcademicCorpus:
    ...
    def test_bad_body_source_still_returned_but_flagged(self):
        """Docs with non-marker body_source are flagged via body_source field.

        In theory these shouldn't reach the KS (academic gate blocks them),
        but if they do, they appear with the correct body_source label.
        """
        ks = _make_ks(academic_docs=[{
        ...
        assert len(result.citations) == 1
        assert result.citations[0].body_source == "pdfplumber"
        # marker_only_count excludes non-marker docs
        assert result.marker_only_count == 0
    ...
    def test_missing_metadata_json_does_not_crash(self):
        """Papers without metadata_json should not crash — use defaults."""
        ks = _make_ks(academic_docs=[{
        ...
        assert not result.had_fallback
        assert result.citations[0].arxiv_id is None
        assert result.citations[0].body_source == "unknown"
