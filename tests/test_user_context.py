...
from polytool.user_context import (
    ...
    get_slug_for_user,
    display_name,
    is_autogenerated_username,
    truncate_wallet,
)


_ADDR = "0xa380c504a480f591c7dfbf9944fac3994b9b21ff"


class TestDisplayName:
    """display_name: prefer a real handle, else fall back to a truncated wallet ID."""

    def test_real_handle_returned(self):
        assert display_name("Scaly-Champion", _ADDR) == "Scaly-Champion"

    def test_strips_leading_at(self):
        assert display_name("@Alice", _ADDR) == "Alice"

    def test_empty_falls_back_to_truncated_wallet(self):
        assert display_name("", _ADDR) == "0xa380...21ff"

    def test_none_falls_back_to_truncated_wallet(self):
        assert display_name(None, _ADDR) == "0xa380...21ff"

    def test_username_equal_to_address_is_autogenerated(self):
        # the old `Username -> address` default must NOT show as a name
        assert display_name(_ADDR, _ADDR) == "0xa380...21ff"

    def test_api_autogenerated_handle_form(self):
        autogen = "0xDc71bED951076a6dfd27f6A341C441B8543ED033-1760135940099"
        wallet = "0xdc71bed951076a6dfd27f6a341c441b8543ed033"
        assert display_name(autogen, wallet) == "0xdc71...d033"

    def test_both_empty_returns_unknown(self):
        assert display_name("", "") == "unknown"

    def test_wallet_id_is_never_the_return_value_as_key(self):
        # canonical key stays the full address; display is intentionally lossy
        d = display_name("", _ADDR)
        assert d != _ADDR and d.startswith("0xa380")


class TestIsAutogeneratedUsername:
    def test_empty_is_autogen(self):
        assert is_autogenerated_username("") is True
        assert is_autogenerated_username(None) is True

    def test_real_handle_not_autogen(self):
        assert is_autogenerated_username("Scaly-Champion") is False

    def test_full_address_is_autogen(self):
        assert is_autogenerated_username(_ADDR) is True

    def test_matches_wallet_id_is_autogen(self):
        assert is_autogenerated_username("ABC", wallet_id="abc") is True

    def test_api_placeholder_form_is_autogen(self):
        assert is_autogenerated_username("0xabc-123") is True


class TestTruncateWallet:
    def test_truncates_long_address(self):
        assert truncate_wallet(_ADDR) == "0xa380...21ff"

    def test_short_value_unchanged(self):
        assert truncate_wallet("0xabc") == "0xabc"
