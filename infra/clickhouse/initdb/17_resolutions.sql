...
CREATE TABLE IF NOT EXISTS polytool.market_resolutions (
    condition_id String,
...
CREATE INDEX IF NOT EXISTS idx_resolutions_token_id ON polytool.market_resolutions (outcome_token_id) TYPE bloom_filter GRANULARITY 1;

-- Index for faster lookups by market_slug
CREATE INDEX IF NOT EXISTS idx_resolutions_market_slug ON polytool.market_resolutions (market_slug) TYPE bloom_filter GRANULARITY 1;

-- View that enriches trades with resolution data
CREATE OR REPLACE VIEW polytool.user_trades_with_resolution AS
SELECT
    t.*,
...
FROM polytool.user_trades_resolved t
LEFT JOIN polytool.market_resolutions r
...
CREATE OR REPLACE VIEW polytool.user_trade_lifecycle AS
SELECT
    proxy_wallet,
    resolved_token_id,
    market_slug,
...
CREATE OR REPLACE VIEW polytool.user_trade_lifecycle_enriched AS
SELECT
    l.*,
    r.settlement_price,
    r.resolved_at,
...
FROM polytool.user_trade_lifecycle l
LEFT JOIN polytool.market_resolutions r
    ON l.resolved_token_id = r.outcome_token_id;

GRANT SELECT ON polytool.market_resolutions TO grafana_ro;
GRANT SELECT ON polytool.user_trades_with_resolution TO grafana_ro;
GRANT SELECT ON polytool.user_trade_lifecycle TO grafana_ro;
GRANT SELECT ON polytool.user_trade_lifecycle_enriched TO grafana_ro;
