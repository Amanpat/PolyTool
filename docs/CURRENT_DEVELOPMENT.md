...
| ------------------------------------------------------------- | ---------- | -------- | ------------------------------------------------------------------ |
| RIS L2 Academic Query — PaperQA2 RAG Control Flow            | 2026-05-09 | RIS      | `docs/features/FEATURE-ris-l2-academic-query.md` — `research-query` CLI; multi-angle KS query (source_family=academic); paper-level grouping; citation output with arxiv_id/body_source; graceful fallback; 34 tests pass. L4 Multi-source Harvesters explicitly deferred (too large). Dev log: `docs/dev_logs/2026-05-09_ris-academic-pipeline-completion-sprint.md`. |
...
- **RIS L2 Academic Query is COMPLETE (2026-05-09).** `research-query` CLI ships multi-angle KS query (source_family=academic, Marker-only via ingest gate), paper-level grouping by doc_id, citation enrichment (title/arxiv_id/source_url/body_source), graceful fallback. 34 tests pass. **ChromaDB academic path deferred** (body_source not in Chroma metadata — future L2.1). **L4 Multi-source Harvesters is explicitly NOT in this sprint** — too large (5 fetchers, session handling, new deps); requires Director workpacket to activate. Feature doc: `docs/features/FEATURE-ris-l2-academic-query.md`. Dev log: `docs/dev_logs/2026-05-09_ris-academic-pipeline-completion-sprint.md`. Active count: 2 (Features 1, 2).
