...
<!-- Feature 3 slot is now FREE — L3.2 completed 2026-05-05. Next recommended: L3 v1 SVM Topic Filter Readiness + Training. -->
...
| Marker Single-Paper Validation Control Surface               | 2026-05-05 | RIS      | `run-academic-url` subcommand; process-boundary subprocess cancel; `parse_seconds` in result; 5 new tests. Validated: `body_source=marker`, `body_length=56923`, `parse_seconds=85.95s`. L1 production still blocked on ≤10s/paper gate. |
...
| ------------------------------------------------------ | -------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| RIS Marker Queue — Docker IPC Warm-Worker (v1)         | 2026-05-05     | Queue v0 shipped. v1 (persistent IPC subprocess, Marker models warm across papers on Linux/Docker) deferred pending operator decision on next packet. | Operator chooses Docker IPC warm-worker packet; validates ≥3 papers warm (≤10s/paper for papers 2+) |
...
- **RIS L3.2 Prefetch Label Discovery Mode is COMPLETE (2026-05-05).** `research-prefetch-discover` CLI shipped: arXiv metadata search → `RelevanceScorer` → `ReviewQueueStore` enqueue; no PDF/Marker/index; 36 tests. **SVM trigger reached: 30 allow / 31 reject / 1 pending unlabeled** (pending does not block readiness). Feature 3 slot is now FREE. **Next recommended Feature 3: L3 v1 SVM Topic Filter Readiness + Training** (SPECTER2 + S2FOS embeddings + SVM classifier, triggered by the ≥30+30 label milestone now met). Dev log: `docs/dev_logs/2026-05-05_l3-2-prefetch-label-discovery-closeout.md`. Work packet status updated to completed: `docs/obsidian-vault/Claude Desktop/12-Ideas/Work-Packet - Prefetch Label Discovery Mode.md`. **Active count is now 2 (Feature 1 + Feature 2) — one Feature 3 slot is available.**
- **REMINDER — Marker Docker/Linux IPC Warm-Worker (Option A) is DEFERRED, NOT CANCELED.** Queue v0 shipped 2026-05-05. v1 (persistent IPC subprocess, Marker models warm across papers on Linux/Docker) must be revisited after this L3/SVM stream completes or before L2 production launch (whichever comes first). Do NOT let this slip past the L3.2 + SVM closeout. See Paused/Deferred table row "RIS Marker Queue — Docker IPC Warm-Worker (v1)".
