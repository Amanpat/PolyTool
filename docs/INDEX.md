...
| [RIS Marker Structural Parser — Production Default (Layer 1)](features/ris-marker-structural-parser-scaffold.md) | **CODE COMPLETE — VALIDATION BLOCKED (2026-05-05).** Marker sole parser, pdfplumber debug-only, `marker_failed` rejection, scheduler split, cache mount corrected. Validation failed: one-shot timeouts on math-heavy papers; scheduler lacks single-paper submit path and process-boundary cancel. Gated on Marker Single-Paper Validation Control Surface work packet. |
| [RIS L3 Pre-fetch Relevance Filter v0 + L3.1](features/FEATURE-ris-prefetch-relevance-filter-v0.md) | Lexical scorer v1.1 (allow=0.80); all four modes: `{off,dry-run,enforce,hold-review}`; `hold-review` queues REVIEW candidates, never ingests; `ReviewQueueStore` + `LabelStore`; `research-prefetch-review` CLI; Scenario B 5.88% (<10% met); QA REJECT=0; SVM trigger: ≥30+30 labels |
