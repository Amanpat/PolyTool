...
| [Vera Discord Bot — Phase A](features/vera_discord_bot_phase_a.md) | **NEW 2026-06-02.** discord.py bot replacing Hermes; Phase A skeleton: `/ping` (ephemeral), `Intents.none()`, token fail-fast (never logged), `Dockerfile.vera` + opt-in `vera-bot` compose service. No writes / no gate access. Phase B (approve/deny buttons) is a separate packet. |
