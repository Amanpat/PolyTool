...
- All 11 workflow JSON files parse as valid JSON
...
- The orphaned `workflows/n8n/` directory (v2 set with wrong container name) does NOT exist
...
4. **Activate desired workflows:**
   ...
   Recommended starting point: `RIS Health Check` and `RIS Scheduler Status`.
...
6. **Test health check workflow manually:**
   In the n8n UI, open the `RIS Health Check` workflow and click "Test workflow".
   Confirm the `research-health` command runs and returns output.

7. **Test webhook with curl:**
   Get the webhook URL from the n8n UI for the `RIS Manual Acquire (Webhook)` workflow,
   then:
   ```bash
...
```
FAIL: correct-container:ris_XXX.json:<node>: Expected 'polytool-ris-scheduler' in command
```
Edit the workflow JSON in `infra/n8n/workflows/` to use `polytool-ris-scheduler`.
The wrong name `polytool-polytool-1` was from an old feature branch.
...
- `docs/features/RIS_OPERATOR_GUIDE.md` -- Full RIS operator guide
