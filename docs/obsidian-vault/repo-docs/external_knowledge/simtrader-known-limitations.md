...
source_quality_caution: Composite document. Section A limitations were verified directly
  against the PolyTool SimTrader codebase. Section B limitations were corroborated
  by external backtesting repo analysis (hermes-pmxt / poly-backtesting-oss). Do not
  attribute all claims to a single source.
source_zone: repo
mirror_of: docs/external_knowledge/simtrader_known_limitations.md
