---
title: Partner Deployment Guide Docker
type: reference
status: active
source_zone: repo
mirror_of: docs/runbooks/PARTNER_DEPLOYMENT_GUIDE_docker.md
...
There is no dedicated crypto-pair CLI kill command in this flow. To immediately
halt the bot without stopping the container, write a truthy value (`1`, `true`,
`yes`, or `on`) to `artifacts/crypto_pairs/kill_switch.txt`. An empty file
created by `touch` does not trip the switch.
...
artifacts/
  tapes/crypto/paper_runs/
    <run-id>/
      ...
      cycle_log.jsonl
  crypto_pairs/
    kill_switch.txt          <- write 1/true/yes/on here to halt the bot
    live_runs/
      <run-id>/
        run_manifest.json
        cycle_log.jsonl
