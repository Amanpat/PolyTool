...
| [RIS L4 Multi-source Academic Harvesters](features/FEATURE-ris-l4-multisource-academic-harvesters.md) | **COMPLETE 2026-05-09.** 4 harvesters: ArxivHarvester, SemanticScholarHarvester, CrossrefHarvester, OpenReviewHarvester. `AcademicCandidate` dataclass + `dedup_candidates()`. `research-harvest` CLI. `SOURCE_CAPABILITY_MATRIX` with SSRN/NBER deferred. 61 tests pass after Codex dedupe-audit fix. |
| [RIS Academic Pipeline — Demo-Ready v1](features/FEATURE-ris-academic-demo-ready-v1.md) | **COMPLETE 2026-05-28.** Developer/operator demo-ready v1: L1-L4 + L2.1 semantic retrieval. Batch B (10 medium papers): done=20, failed=0, sidecar_count=20; Chroma 917 chunks / 21 papers / 0 orphans. Codex PASS WITH CONCERNS. Caveats: weather lexical false positive, Docker Chroma gap, JIT cache unresolved, Batch C/D deferred. |
| [RIS L2 Academic Query — PaperQA2 RAG Control Flow](features/FEATURE-ris-l2-academic-query.md) | **COMPLETE 2026-05-09 (L2.1 semantic path added 2026-05-25).** `research-query` CLI; multi-angle KS query; ChromaDB semantic vector search primary (L2.1); KS lexical fallback; query-time Marker-ready guard; paper-level grouping; citation output; 54 tests pass. |
...
| [Vera Discord Bot (canonical)](features/FEATURE-vera-discord-bot.md) | **SHIPPED 2026-06-02.** The purpose-built discord.py bot (NOT the retired `vera-hermes-agent`). Webhook notifies, bot handles interaction. Phase A (`/ping`) + Phase B (`/pending` + approve/deny buttons, thin trigger over `discovery review`, author-guard, idempotency). Codex 10/10 PASS; live-verified. Consolidates the two phase docs below. |
| [Vera Discord Bot — Phase A](features/vera_discord_bot_phase_a.md) | **NEW 2026-06-02.** discord.py bot replacing Hermes; Phase A skeleton: `/ping` (ephemeral), `Intents.none()`+guilds, token fail-fast (never logged), `Dockerfile.vera` + opt-in `vera-bot` compose service. No writes / no gate access. |
| [Vera Discord Bot — Phase B](features/vera_discord_bot_phase_b.md) | **NEW 2026-06-02.** First write surface: `/pending` + one-tap approve/deny buttons (operator-only, ephemeral). THIN TRIGGER over the verified `discovery review` CLI (never writes rows directly). Author-guard on list+click, full-address re-validation, list-form subprocess (no shell), in-process idempotency (no double-write). Codex adversarial review: 10/10 invariants PASS. |
...
| [Academic Scaled Validation — Batch B Post-Parse](dev_logs/2026-05-28_academic-scaled-validation-batch-b.md) | 2026-05-28 | Batch B validation: done=20, failed=0, sidecar_count=20. 10/10 marker_ready=True, ipc_warm_worker_used=True. Docker lacks chromadb; Windows host fallback used. Chroma: 917 chunks / 21 papers / 0 orphans. 7 topic probes had_fallback=False, retrieval_mode=semantic. Weather false positive caveat documented. |
| [Codex Review — Academic Batch B Closeout](dev_logs/2026-05-28_codex-review-academic-batch-b-closeout.md) | 2026-05-28 | PASS WITH CONCERNS. Independent verification: parse evidence matches, Chroma clean, 5 probes confirm expected Batch B papers at semantic rank. Weather false positive non-blocking for demo-ready v1. |
| [RIS Academic Query — Natural-Language Normalization: Docs Sync](dev_logs/2026-05-09_ris-academic-query-natural-language-normalization-docs-sync.md) | 2026-05-09 | Docs-only closeout sync. `_normalize_question()` + `_build_sub_queries()` strip question preambles for retrieval. JSON output preserves original question. `test_research_query.py` = 54 passed. Primary retrieval is now ChromaDB semantic (L2.1 complete 2026-05-25). |
| [RIS Academic Pipeline — 3-Paper Operator Validation](dev_logs/2026-05-09_ris-academic-pipeline-3paper-operator-validation.md) | 2026-05-09 | Docs-only validation record. Functional end-to-end pass: 3 arXiv papers; queue 3 done / 0 failed; 79 chunks / 373 claims; `research-query` `had_fallback=false`. Windows/local warm-thread path (`ipc_warm_worker_used=false`). Docker/GPU IPC batch is optional performance follow-up. Academic pipeline is operator-tested v1. |
