---
title: "Work Packet — Scientific RAG Evaluation Benchmark v0"
type: work_packet
status: active
...
1. **`[[legacy/Claude Desktop/08-Research/11-Scientific-RAG-Pipeline-Survey]]`** — three entries are directly relevant:
...
2. **`[[legacy/Claude Desktop/09-Decisions/Decision - Scientific RAG Architecture Adoption]]`** — item 5 in "Adopt" specifies this layer's purpose. Constrains scope to evaluation, explicitly NOT a new RAG pipeline.
...
4. **`[[legacy/Claude Desktop/09-Decisions/Decision - RIS Evaluation Scoring Policy]]`** — establishes how the LLM eval gate scores documents. Inform the metric definitions (especially metric 1, off-topic rate) — there should be no contradiction between how this benchmark measures relevance and how the gate scores it.
...
- **Multi-model eval gate decision is gated on L5 output.** The decision document `[[legacy/Claude Desktop/09-Decisions/Decision - RIS Evaluation Gate Model Swappability]]` already establishes the swappability infrastructure; adding a second model becomes a small follow-up packet if L5 says it's needed.
...
- [[Claude Desktop/09-Decisions/Decision - Scientific RAG Architecture Adoption]] — adoption decision (item 5)
- [[claude-memory/work-packets/work-packet-academic-pipeline-pdf-download-fix]] — Layer 0 (shipped); provides the corpus this packet measures
- [[claude-memory/work-packets/work-packet-marker-structural-parser-integration]] — Layer 1 (parallel); benchmark will be re-run after Marker ships to measure improvement
- [[claude-memory/work-packets/work-packet-paperqa2-rag-control-flow]] — Layer 2 (gated on this packet's findings)
- [[claude-memory/work-packets/work-packet-pre-fetch-svm-topic-filter]] — Layer 3 (activation conditional on this packet's off-topic-rate metric)
- [[claude-memory/work-packets/work-packet-multi-source-academic-harvesters]] — Layer 4 (downstream)
- [[Claude Desktop/08-Research/11-Scientific-RAG-Pipeline-Survey]] — SciQAG and SciDQA entries
- [[Claude Desktop/09-Decisions/Decision - RIS Evaluation Scoring Policy]] — scoring policy this benchmark measures against
- [[Claude Desktop/09-Decisions/Decision - RIS Evaluation Gate Model Swappability]] — multi-model decision gated on this benchmark
