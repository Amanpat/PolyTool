---
title: "Work Packet — Academic Pipeline Scaled Validation Corpus"
type: work_packet
status: complete
...
> [!SUCCESS] Status: COMPLETE — Academic RIS Developer/Operator Demo-Ready v1 FORMALLY CLOSED 2026-05-28
> Batch A (5 small papers) + Batch B (10 medium papers) completed. Queue: 20 done, 0 failed,
> 20 sidecars. Chroma: 917 chunks / 21 papers / 0 orphans. 7 semantic probes passed.
> Final Codex review: PASS (2026-05-28). Feature doc: `docs/features/FEATURE-ris-academic-demo-ready-v1.md`.
> **NOT production-ready.** Caveats: weather lexical false positive, Docker Chroma gap,
> JIT cache persistence unresolved, Batch C/D deferred to post-v1 hardening (Tier-3 approval required).
> Next Academic RIS work: post-v1 hardening only — first item is Docker `jit-cache-check`.
> Closeout dev log: `docs/dev_logs/2026-05-28_academic-ris-demo-ready-v1-closeout.md`

> [!WARNING] Scope Guard (historical — work packet closed; kept for record)
> - ~~Do NOT implement L2 semantic/vector retrieval (deferred as L2.1)~~ — **L2.1 ChromaDB semantic retrieval COMPLETE 2026-05-25**
> - Do NOT enable SVM enforce mode — hard-blocked pending Director approval (still applies)
> - Do NOT add SSRN/NBER harvesters — deferred per SOURCE_CAPABILITY_MATRIX (still applies)
> - Do NOT validate Docker IPC batch performance — separate optional infra follow-up (still applies)
> - Do NOT bulk-accept QA pairs — each pair requires individual operator verification (still applies)
> - Do NOT claim academic pipeline production-ready from this run alone (still applies — pipeline is demo-ready, NOT production-ready)
...
| Prohibited | Reason |
|-----------|--------|
| ~~L2 semantic/vector retrieval (ChromaDB academic path)~~ | ~~Deferred as L2.1~~ — **COMPLETE 2026-05-25** (`_embed_body_into_chroma()`, `--reindex-chroma`, `check-chroma-links` shipped; semantic search is now primary retrieval path) |
