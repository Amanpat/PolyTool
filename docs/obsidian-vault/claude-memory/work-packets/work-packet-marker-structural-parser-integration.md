---
title: "Work Packet — Marker Structural Parser Integration (Production Rollout)"
type: work_packet
status: active
...
> - New packet created: [[claude-memory/work-packets/work-packet-marker-canonical-academic-parse-queue]] (status: ready)
>
> ~~**This packet resumes when [[claude-memory/work-packets/work-packet-marker-canonical-academic-parse-queue]] ships.**~~ **Queue shipped. Feature 3 closed 2026-05-08. L1 warm-worker blocker resolved; next L1 rollout/readiness step requires separate workpacket/Director decision.**
>
...
Layer 1 of the [[Claude Desktop/08-Research/11-Scientific-RAG-Target-Architecture|four-layer scientific RAG target]].
...
1. **`[[legacy/Claude Desktop/08-Research/11-Scientific-RAG-Pipeline-Survey]]`** — the Marker entry has the full evaluation including license analysis, GPU/CPU performance characterization, and recommended schemas for tables/equations/sections. This is the primary reference.
2. **`[[legacy/Claude Desktop/09-Decisions/Decision - Scientific RAG Architecture Adoption]]`** — item 1 in "Adopt" specifies Marker as primary parser. License posture (research/personal under modified Open Rail-M) is established here.
3. **`[[claude-memory/decisions/decision-academic-pipeline-hosting]]`** — must be created and answered before this packet ships. Determines which machine runs the pipeline.
...
1. ~~**Hosting answer.**~~ **RESOLVED 2026-05-02.** Docker + GPU passthrough on dev machine. RTX 2070 Super, CUDA 13.2. Model weights volume-mounted from `~/.cache/datalab/`. See [[claude-memory/decisions/decision-academic-pipeline-hosting]] (status: accepted).
...
- [[Claude Desktop/09-Decisions/Decision - Academic Pipeline Hosting]] — prerequisite decision
- [[claude-memory/work-packets/work-packet-academic-pipeline-pdf-download-fix]] — Layer 0 (shipped); pdfplumber path retired by this packet
- [[claude-memory/work-packets/work-packet-paperqa2-rag-control-flow]] — Layer 2; consumes Marker's structured output
- [[Claude Desktop/08-Research/11-Scientific-RAG-Pipeline-Survey]] — Marker entry has the full evaluation
