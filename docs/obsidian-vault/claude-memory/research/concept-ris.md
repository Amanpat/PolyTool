---
title: "Research Intelligence System (RIS)"
type: concept
status: superseded
superseded_by: repo-docs/specs/spec-ris-phase2-operational-contracts.md
...
**Packaging gap:** `evaluation`, `ingestion`, `integration`, `monitoring`, and `synthesis` subpackages are NOT registered in `pyproject.toml`. They work via `sys.path` insertion but would not install correctly as a proper Python package. See [[legacy/PolyTool/07-Issues/Issue-Pyproject-Packaging-Gap]].
...
Note: There is also a JSON-backed hypothesis registry at `packages/polymarket/hypotheses/`. See [[legacy/PolyTool/07-Issues/Issue-Duplicate-Hypothesis-Registry]].
...
- [[legacy/PolyTool/02-Modules/RAG]] — ChromaDB + SQLite FTS5 retrieval layer
- [[legacy/PolyTool/01-Architecture/LLM-Policy]] — Evaluation uses Tier 1 free LLM providers
- [[legacy/PolyTool/07-Issues/Issue-Pyproject-Packaging-Gap]] — 5 subpackages missing from pyproject.toml
- [[legacy/PolyTool/07-Issues/Issue-Duplicate-Hypothesis-Registry]] — JSON-backed vs SQLite-backed registry
- [[claude-memory/decisions/decision-ris-n8n-pilot-scope]] — Pilot boundary, canonical paths, APScheduler default
