...
## [2026-06-01] wallet-ingestion-v1 | WI-4 Two-Tier Watchlist COMPLETE
Added `tier`(candidate|locked)+`locked` columns (kept existing `source` origin meaning; matches WI-3 `resolve_tier`), candidate-tier auto-population from scan evidence, locked-tier immutability (byte-identical across a full cycle), deterministic `summarize_evidence()` reason string (the WI-5 contract), and `discovery review --approve/--deny` over the enforced human gate (no auto-promote). Live-CH watchlist ALTER applied + verified (smoke row → candidate/0). 38 new tests. Commit `c31d59c`.
Packet: [[claude-memory/work-packets/work-packet-wi-4-two-tier-watchlist]] — Dev log: docs/dev_logs/2026-06-01_wi-4-two-tier-watchlist.md

## [2026-06-04] day-run | DR-1 One-Command On/Off Toggle COMPLETE (code)
Canonical `scripts/scan.sh {on|off|status}` wrapper: `on` = `docker compose up -d clickhouse api discovery-scheduler` (idempotent); `off` = `docker compose stop discovery-scheduler` only (ClickHouse + API stay up, honoring the DR-0 graceful SIGTERM stop); `status` via `scripts/scan_status.py` reusing existing readers (`ScanQueueManager.load_from_clickhouse`/`get_pending` + `read_pending_candidates`). Central safety property: the wrapper NEVER runs `docker compose down` and NEVER passes `-v`/`--volumes` — actively refuses teardown tokens (verified live + by test). 12 new tests (147 in focused subset, 0 failures). Full live on→status→off smoke BLOCKED (no running stack in env) — operator steps in the dev log.
Packet: [[claude-memory/work-packets/work-packet-dr-1-onoff-toggle]] — Dev log: docs/dev_logs/2026-06-04_dr-1-onoff-toggle.md
